from sqlgrid import sqlgridWidget, set_defaults, show_grid, on as sqlgrid_on
...
def init_event_history(event_names, widget=None):
    ...
    def on_change(event, sqlgrid_widget):
        event_history.append(event)

    if widget is not None:
        widget.on(event_names, on_change)
    else:
        sqlgrid_on(event_names, on_change)

    return event_history


def test_edit_date():
    view = sqlgridWidget(df=create_df())
    check_edit_success(
        view,
        "Date",
        3,
...
def test_edit_multi_index_df():
    ...
    view = sqlgridWidget(df=df_multi)
...
def test_add_row_button():
    widget = sqlgridWidget(df=create_df())
    event_history = init_event_history("row_added", widget=widget)

    widget._handle_sqlgrid_msg_helper({"type": "add_row"})
...
def test_remove_row_button():
    ...
    widget._handle_sqlgrid_msg_helper({"type": "remove_row"})
...
def test_mixed_type_column():
    ...
    view._handle_sqlgrid_msg_helper(
...
def test_row_edit_callback():
    ...
    view = sqlgridWidget(df=sample_df, row_edit_callback=can_edit_row)
...
def test_get_selected_df():
    sample_df = create_df()
    selected_rows = [1, 3]
    view = sqlgridWidget(df=sample_df)
    view._handle_sqlgrid_msg_helper(
        {"rows": selected_rows, "type": "change_selection"}
    )
...
def test_multi_index():
    widget = sqlgridWidget(df=create_multi_index_df())
    event_history = init_event_history(
...
def test_change_viewport():
    widget = sqlgridWidget(df=create_large_df())
...
def test_change_selection():
    widget = sqlgridWidget(df=create_large_df(size=10))
    event_history = init_event_history("selection_changed", widget=widget)

    widget._handle_sqlgrid_msg_helper({"type": "change_selection", "rows": [5]})
